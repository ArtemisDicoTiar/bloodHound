...
trait Tables {
  ...
  class Baskets(_tableTag: Tag) extends profile.api.Table[BasketsRow](_tableTag, Some("coupang"), "baskets") {
    ...
    lazy val productsFk = foreignKey("productId", prodid, Products)(r => r.prodid, onUpdate=ForeignKeyAction.Cascade, onDelete=ForeignKeyAction.Cascade)
  }
  ...
  class Prices(_tableTag: Tag) extends profile.api.Table[PricesRow](_tableTag, Some("coupang"), "prices") {
    ...
    lazy val productsFk = foreignKey("prodId", prodid, Products)(r => r.prodid, onUpdate=ForeignKeyAction.Cascade, onDelete=ForeignKeyAction.Cascade)
  }
   ...
   *  @param thumbnailurl Database column thumbnailUrl SqlType(VARCHAR), Length(500,true), Default(None) */
  case class ProductsRow(prodid: Int, name: String, url: String, thumbnailurl: Option[String] = None)
  ...
  implicit def GetResultProductsRow(implicit e0: GR[Int], e1: GR[String], e2: GR[Option[String]]): GR[ProductsRow] = GR{
    prs => import prs._
    ProductsRow.tupled((<<[Int], <<[String], <<[String], <<?[String]))
  }
  ...
  class Products(_tableTag: Tag) extends profile.api.Table[ProductsRow](_tableTag, Some("coupang"), "products") {
    def * = (prodid, name, url, thumbnailurl) <> (ProductsRow.tupled, ProductsRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(prodid), Rep.Some(name), Rep.Some(url), thumbnailurl).shaped.<>({ r=>import r._; _1.map(_=> ProductsRow.tupled((_1.get, _2.get, _3.get, _4)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    /** Database column thumbnailUrl SqlType(VARCHAR), Length(500,true), Default(None) */
    val thumbnailurl: Rep[Option[String]] = column[Option[String]]("thumbnailUrl", O.Length(500,varying=true), O.Default(None))
